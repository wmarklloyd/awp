...
The active 0.8 direction uses one canonical Markdown capsule, `<project-name>.awp.md`, inside a declared project. That Capsule is the normative project entry point and identifies its governing specification and every required external artifact or binding through its metadata, manifest, and records. Standalone capsule exchange, editable-directory packages, ZIP packages, and JSON wire payloads are archived directions; they are not active 0.8 collaboration or conformance profiles.
...
After a successful projection, a host can derive a bounded in-memory participant presentation from the already validated proposed state rather than reparsing the new Capsule and reverifying unchanged artifacts. That presentation remains ephemeral, has no independent project filename or lifecycle, and gains no authority. A participant-facing `complete` result still depends on all validation required by Sections 3.1 and 3.2.
...
## 18.1 Presence, monitoring, and scale

Agent presence monitoring and the scaling requirements for large or multi-host
deployments are specified in [AWP Coordination Scale](coordination-scale.md).
They are COOP-3 concerns: a COOP-1 or COOP-2 binding composes without them, and
a binding that claims presence monitoring or the scaling profile MUST satisfy
that document. Advisory participant liveness leases, which COOP-1 does use, are
specified in section 19 below and are not the same mechanism.
...
| Coordination schema | `schemas/awp-coordination-0.5.schema.json` | 20763 | `55cc2ac57365d32642d6b323adefdc843ae7e40692516141eb8ab30ca56b0e91` |
| Security schema | `schemas/awp-security-0.5.schema.json` | 2455 | `d0d9a357a8d9aafa2994659fc639354f859b55b9286469d3193b3e08ef94d21b` |
| Module-registry schema | `schemas/awp-module-registry-0.8.schema.json` | 2887 | `09e584443dd107f1d760a162c486a15257adcc29cf4b91884a7a699b9b576748` |
