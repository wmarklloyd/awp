# Agent Workstate Protocol 0.5.0 — Complete Specification Bundle

**Status:** Generated distribution artifact  
**Source of truth:** `AWP_SPECIFICATION_0.5.0.md` and `spec/0.5.0/*.md`  
**Purpose:** Self-contained copy for agents or systems that cannot follow repository-relative links.

This file bundles the 0.5.0 family overview and every published module specification. Module boundaries and independent versions remain normative. Do not edit this generated file directly; regenerate it from the source files when the specification changes.
...
AWP 0.5.0 does not define destructive log compaction. A writer MAY create a summary or snapshot-only export, but it MUST disclose omitted history and MUST NOT claim `full` completeness.
...
No single mapping is normative in 0.5.0. Branches and pull requests are forge conventions rather than universal Git objects. Git object IDs establish repository object identity, not semantic safety, actor authority, or AWP event identity.
