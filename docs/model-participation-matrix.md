...
| MP-05 | Compare current work with known participants and dependencies | `announce`, `read`, `interact` | Scope expansion and requested context | Projector and coordination evaluator | Physical/declared overlap results with coverage and freshness | `warning` or `unverifiable`; never silent no-conflict |
| MP-06 | Record a conflict, acknowledgement, order, or user question | `interact` | Proposed disposition, rationale, evidence, requested decision | Adapter, projector, and policy binding | Durable interaction state and any pending authority requirement | `needs_input`, `blocked`, or `unverifiable` |
...
| MP-15 | Handle incomplete or unknown history | `read`, `interact`, `publish` | Choice to request another page or stop | Projector and adapter | Missing ancestry, retention gap, or unsupported semantics exposed | `unverifiable` with recovery action |
