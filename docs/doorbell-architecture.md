...
Claude in the desktop app (Cowork) has no endpoint that a local process can call: the scheduled task a Cowork session creates is not listed on the Routines screen and so cannot get an API trigger, and its sandbox cannot reach third-party push services (checked 2026-09-11). It can, however, run a background hook that wakes the idle session when the hook exits with code 2 (`asyncRewake`, verified 2026-09-11). Entering with `--host cowork` therefore declares a `git-signal` W1 binding when the project has an https origin: the relay publishes a content-free commit, whose message names only the recipient and event identifiers, to `refs/awp/wake/<actor token>`, and the session's hook (`tools/awp_wake_watch.sh`) watches that one ref and wakes the session when it changes; the agent then runs `awp_wake pending --ack`. Waiting costs no model turns. Without a readable remote the binding falls back to `self-poll`. Because the signal is on the project's remote, anyone who can read that remote can see that an event was addressed to an agent and when, but not what it says. Probe TTLs default to the recipient's first rung window (`suggested_ttl`).
