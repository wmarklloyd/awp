...
Claude in the desktop app (Cowork) cannot be woken from outside: a Cowork session has no local endpoint, the scheduled task a Cowork session creates is not listed on the Routines screen and so cannot get an API trigger, and its sandbox cannot reach third-party push services (checked 2026-09-11). It can, however, schedule durable wake-ups of itself. Entering with `--host cowork` therefore declares a `self-poll` W1 binding (`interval_seconds` 120, window 240 s): while the session is active it wakes every two minutes, runs `awp_wake pending --ack`, and reschedules. The relay records its attempt, waits the window, and escalates to the notification if no receipt arrives, and the declaration probe measures the real latency. Probe TTLs default to the recipient's first rung window (`suggested_ttl`). A hosted wake (W3) remains an opt-in for agents whose vendor offers an API trigger.
