#!/usr/bin/env python3
"""PostToolUse binding: recompute the invocation binding against the tool
call Codex says actually ran, using tools/awp_harness.py's `enforce`.
Cannot block -- the tool already executed by the time PostToolUse fires. A
mismatch here means Stage 2's gate approved a different call than the one
that actually ran (TOCTOU) -- this cannot undo the tool call, but it is what
feeds tools/awp_ci_gate.py's later, blocking check an honest signal.

Same payload/output contract as ../claude-code-hooks/posttooluse.py (see
that file and ./pretooluse.py's docstring for the sourcing and the coverage
caveat -- PostToolUse is documented to fire for the same "Bash-like" local
tool calls as PreToolUse, so it verifies exactly what pretooluse.py gated,
no more).
"""
...
def main() -> int:
    ...
    pending_dir = _REPO_ROOT / config.get("pending_dir", ".awp-runtime/pending/codex")
    pending_path = pending_dir / f"{tool_use_id}.json"
    if not pending_path.exists():
        # No PreToolUse resolution was recorded for this call (untracked
        # tool, or the classifier/mapping did not match) -- nothing to verify.
        print(json.dumps({"hookSpecificOutput": {"hookEventName": "PostToolUse"}}))
        return 0
