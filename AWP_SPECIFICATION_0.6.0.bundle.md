...
# Agent Workstate Protocol 0.6.0

**Status:** Exploratory modular draft  
**Published:** 2026-09-03  
**Supersedes:** AWP 0.5.0  
**Normative language:** MUST, MUST NOT, REQUIRED, SHOULD, SHOULD NOT, and MAY express requirement levels.

## 1. Purpose

AWP is a family of composable specifications for preserving, exchanging, inspecting, and resuming work performed by humans and software agents. Version 0.6.0 integrates Coordination 0.3.0: a deterministic, tiered protocol for durable work intents, semantic overlap, bounded negotiation, contracts, typed preconditions, verification-bound change sets, staleness propagation, integration, and optional live enforcement. Repository discovery and project re-entry from 0.5.0 remain part of the family.
...
|---|---|---:|---|---|
| [AWP Core](spec/0.6.0/core.md) | `urn:awp:core` | `0.6.0` | required | none |
| [AWP Capsule](spec/0.6.0/capsule.md) | `urn:awp:capsule` | `0.3.0` | optional | Core |
| [AWP Handoff](spec/0.6.0/handoff.md) | `urn:awp:handoff` | `0.3.0` | optional | Core |
| [AWP Artifact](spec/0.6.0/artifact.md) | `urn:awp:artifact` | `0.3.0` | optional | Core |
| [AWP Synchronization](spec/0.6.0/synchronization.md) | `urn:awp:sync` | `0.3.0` | optional | Core |
| [AWP Coordination](spec/0.6.0/coordination.md) | `urn:awp:coordination` | `0.3.0` | experimental | Core, Synchronization |
| [AWP Security](spec/0.6.0/security.md) | `urn:awp:security` | `0.3.0` | optional | Core; Artifact when artifact controls are used |
| [AWP Adapter Framework](spec/0.6.0/adapters.md) | not a payload module | `0.3.0` | informative | binding-specific |

The machine-readable [module registry](spec/0.6.0/modules.json) is normative for the module IDs, versions, document paths, stability labels, and direct dependencies in this release.

## 3. Module declarations

Every AWP 0.6 manifest MUST contain a `modules` array. It MUST declare exactly one Core entry, and that entry MUST be required. The following is a module-declaration excerpt rather than a complete manifest:
...
```json
{
  ...
  "modules": {
    "urn:awp:core": ["0.6.x"],
    "urn:awp:capsule": ["0.3.x"],
    "urn:awp:handoff": ["0.3.x"]
  },
...
## 9. Migration from 0.5.0

AWP 0.6.0 preserves the 0.5 event envelope and module identifiers. Core advances to `0.6.0`; the carried-forward optional modules advance to `0.3.0` to declare compatibility with this Core release. Their principal semantics remain those of the 0.5 family unless their documents state otherwise.

Coordination 0.3.0 replaces Coordination 0.2.0. It adds C0–C3 conformance levels, capability bundles, revision-pinned references, deterministic reconciliation, a semantic registry, declared-versus-observed scopes, bounded negotiation and commitments, machine-readable contract decisions, typed preconditions, verification binding, dependency-driven staleness, partial-integration semantics, diagnostics, and fenced live coordination.

An upgrader from 0.5.0 MUST update declared module versions. Coordination 0.2 records require semantic migration rather than a version-number substitution because record types, lifecycle rules, reference pinning, and readiness semantics changed. Historical 0.2 events remain historical assertions and SHOULD be preserved; an upgrader emits 0.3 successor or import events and records any information that could not be mapped.

## 10. Release contents

- [Core schema](schemas/awp-core-0.6.schema.json)
...
**Status:** Required  
**Dependencies:** None  
**Schema:** `../../schemas/awp-core-0.6.schema.json`
...
The Core module declaration MUST appear exactly once with version `0.6.x` and `required: true`. Module IDs MUST be unique within the array. A module declaration MUST satisfy the dependency and requiredness rules in the family specification.
...
AWP 0.6.0 does not define destructive log compaction. A writer MAY create a summary or snapshot-only export, but it MUST disclose omitted history and MUST NOT claim `full` completeness.
...
**Status:** Optional  
**Depends on:** AWP Core `0.6.x`; AWP Artifact `0.3.x` when `artifact-controls` is declared
...
AWP Security 0.3.0 does not select a normative canonicalization or signature algorithm. Implementations MUST NOT claim interoperable AWP signature conformance without naming an external or future registered signature profile.
...
```json
{
  "$schema": "../../schemas/awp-module-registry-0.6.schema.json",
  "family": "AWP",
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:awp:schema:core:0.6.0",
  "title": "AWP Core 0.6",
  "description": "Structural schema for AWP 0.6 manifests, event envelopes, snapshots, actors, authority declarations, and Core records.",
  "oneOf": [
    { "$ref": "#/$defs/manifest" },
    { "$ref": "#/$defs/event" },
    { "$ref": "#/$defs/snapshot" },
  ...
  "$defs": {
    ...
    "manifest": {
      ...
      "properties": {
        "awp_version": { "type": "string", "pattern": "^0\\.6\\.[0-9]+$" },
        ...
        "modules": {
          ...
          "contains": {
            ...
            "properties": {
              ...
              "version": { "type": "string", "pattern": "^0\\.6\\.[0-9]+$" },
              "required": { "const": true }
            }
          },
          "minContains": 1,
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:awp:schema:coordination:0.3.0",
  "title": "AWP Coordination 0.3",
  "description": "Structural schema for AWP Coordination 0.3 records and event envelopes.",
  "oneOf": [
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:awp:schema:module-registry:0.6.0",
  "title": "AWP 0.6 Module Registry",
  "type": "object",
  "required": ["family", "family_version", "event_schema_versions", "modules"],
  "properties": {
    "$schema": { "type": "string" },
    "family": { "const": "AWP" },
    "family_version": { "const": "0.6.0" },
...
```json
{
  ...
  "properties": {
    ...
    "awp_discovery_version": { "const": "0.1" },
