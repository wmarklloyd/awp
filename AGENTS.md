...
The current project goal is seamless inter-agent collaboration: after a user authorizes a bounded COOP-2 interaction, the initiating agent should be able to reach another available agent in the same project without requiring the user to relay paths, messages, or reminders. The complete AWP 0.8.0 working-draft bundle is the sole source of operative requirements, including the distinction between publication, delivery, and answer and the boundary between the project mailbox and an optional host wake mechanism.
