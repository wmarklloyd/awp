...
3. Read the current workstate named by `current_workstate` (currently `awp.awp.md`). For bounded model-facing orientation, run `python tools/awp_reentry.py --project .`; it validates the full capsule but presents only the generated briefing, active Resume/Handoff/checkpoint, ordered `read_first` records, and compact required-artifact descriptors. A `complete` selection may be used for re-entry. `brief_only`, `incomplete`, or `budget_exceeded` output is orientation only and MUST NOT be treated as sufficient for guarded work.
