...
How you can be woken is declared as wake bindings. Nobody configures them: entering the project declares whatever your environment offers (your live session, a headless run when your command-line tool is installed on the relay's machine, and a desktop notification to the principal as the last resort), and the relay verifies each by probe. Session activation also starts or attaches to the one relay for this clone. `python -m tools.awp_wake list|declare|probe` exist for inspection and custom adapters. A hosted agent that nothing outside can wake (Claude in Cowork) enters with `--host cowork`. When the project has an https origin it declares a `git-signal` binding: the relay publishes a content-free commit (recipient and event identifiers only) to `refs/awp/wake/<actor token>` on that remote, and the agent runs `tools/awp_wake_watch.sh <url> <ref> <actor>` as a background hook that wakes it when the ref changes (for Claude: a Stop hook with `"asyncRewake": true`). `python -m tools.awp_wake enter --actor <self> --host cowork` prints the two commands to run next: `arm_watcher` (in the agent's own workspace; it installs the subscription hook) and `verify` (a probe through that binding, so each session's path is verified rather than assumed). On waking it runs `python -m tools.awp_wake pending --actor <self> --ack`. Without a readable remote it falls back to a two-minute `self-poll` binding. The relay waits the binding's window before escalating to the principal notification. Before sending a consultation, `python -m tools.awp_wake reach --actor <recipient>` states which rung the recipient will get; `python -m tools.awp_wake outcome --event <event>` states how a sent item ended. Credentials for hosted wakes are referenced (`env:NAME` or `file:~/.awp/secrets/NAME`), never written into the ledger, repository, or notices.
