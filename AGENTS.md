...
4. Consultation is disabled by default. If the project explicitly enables review, critique, alternative-model, delegation, decision, or synthesis, declare the purpose, decision owner, and effective loop policy. Do not continue after its limit without the decision owner's recorded continuation.
   For managed COOP-2 collaboration, use the project-scoped rendezvous defined in `spec/drafts/0.8.0/cooperation-contracts.md` §5.1. Participants register and discover one another through the shared `(project_id, workstate_id, binding identity)` tuple and exchange durable interaction IDs and receipts through the binding; do not require the user to pass artifact paths between sessions. Discovery does not widen authorization: only named or explicitly bounded participants may receive work-affecting requests. If participant discovery or interaction retrieval is unavailable or unverifiable, disclose that state and do not imply that a direct agent channel exists.
