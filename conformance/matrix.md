...
| Cooperation Contract COOP-1 | Cooperation Contracts 0.1 draft | Binding, interaction, and participant-lease schema fixtures | 1 simulated shared-binding guarded-work trial plus 1 bounded cross-model review using one store across different execution paths; component tests | 0 | Experimental profile only; the live review's exit ordering failed Section 4.4, so canonical checkpoint projection, recoverable exit, fresh-entry evidence, and complete composition remain outstanding |
| Simpler model participation adapter | Experimental participation contract 0.1 | 3 request/response/receipt examples | 14 local read/announce/publish/interact/checkpoint, idempotency, overlap, scope, freshness, and contract-validation tests | 0 | Continue-checkpoint slice only; exit checkpointing, host authority, and complete C1 composition remain outstanding |
